...
if (isset($_GET['delete'])) {
    ...
    $getFlower = $conn->prepare("SELECT flowerName FROM Sympathy WHERE id = ? AND owner_id = ?");
    ...
    if ($flowerRow = $flowerRes->fetch_assoc()) {
        ...
        $stmtDel = $conn->prepare("DELETE FROM Sympathy WHERE id = ? AND owner_id = ?");
...
$sql = "SELECT id, flowerName, price, image_path, availability FROM Sympathy WHERE owner_id = ?";
$params = [$owner_id];
$types = "i";
...
<title><?= $shopName ?> - Sympathy Flowers</title>
...
<div class="main-content">
  <h2>Flower Category Sympathy</h2>
...
<div class="header-top" style="margin-top:100px;">
  ...
  <a href="manageSympathy.php" class="add-flower-btn"><i class="fa-solid fa-plus"></i> Add Flower</a>
</div>
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        <div class='product-card'>
            ...
            <a href='manageSympathy.php?edit=$id' class='edit-btn'><i class='fa-solid fa-pen'></i> Edit</a>
            <a href='#' class='delete-btn' onclick='openModal($id)'><i class='fa-solid fa-trash'></i> Delete</a>
        </div>";
    }
} else {
    echo "<p style='text-align:center; color:#ccc; grid-column:1/-1;'>No Sympathy flowers found.</p>";
}
?>
</section>
