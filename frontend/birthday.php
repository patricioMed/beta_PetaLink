...
<title>PetaLink - Birthday Flowers</title>
...
<header>
  ...
  <div class="icons">
    <form method="GET" action="Birthday.php" class="search-bar">
...
<div class="navbar">
  ...
  <a style="border-bottom:3px solid #5b2c6f;" href="birthday.php">Birthday</a>
...
if ($res && $res->num_rows > 0) {
  while($row = $res->fetch_assoc()) {
    ...
    <div class="product-card">
      <a href="'.($row['availability'] == 1 ? 'details.php?id='.$id.'&category=Birthday' : '#').'" '.($row['availability']==0?'style="pointer-events:none;opacity:0.6;cursor:not-allowed;"':'').'>
      ...
      <div class="product-info">
        ...
        <a class="view-button" href="'.($row['availability']==1?'details.php?id='.$id.'&category=Birthday':'#').'" '.($row['availability']==0?'style="pointer-events:none;opacity:0.5;cursor:not-allowed;"':'').'>View Details</a>
      </div>
    </div>';
  }
} else {
