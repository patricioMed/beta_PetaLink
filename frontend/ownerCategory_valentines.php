...
if (isset($_GET['delete'])) {
    ...
    $getFlower = $conn->prepare("SELECT flowerName FROM Valentines WHERE id = ? AND owner_id = ?");
    ...
    if ($flowerRow = $flowerRes->fetch_assoc()) {
        ...
        $stmtDel = $conn->prepare("DELETE FROM Valentines WHERE id = ? AND owner_id = ?");
...
$sql = "SELECT id, flowerName, price, image_path, availability FROM Valentines WHERE owner_id = ?";
$params = [$owner_id];
$types = "i";
...
<div class="main-content">
  <h2>Flower Category Valentines</h2>
...
<div class="header-top" style="margin-top:100px;">
  ...
  <a href="manageValentines.php" class="add-flower-btn"><i class="fa-solid fa-plus"></i> Add Flower</a>
</div>
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        <div class='product-card'>
            ...
            <a href='manageValentines.php?edit=$id' class='edit-btn'><i class='fa-solid fa-pen'></i> Edit</a>
            <a href='#' class='delete-btn' onclick='openModal($id)'><i class='fa-solid fa-trash'></i> Delete</a>
        </div>";
    }
} else {
    echo "<p style='text-align:center; color:#ccc; grid-column:1/-1;'>No Valentines flowers found.</p>";
}
?>
</section>
