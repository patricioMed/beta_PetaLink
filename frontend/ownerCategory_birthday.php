...
if (isset($_GET['delete'])) {
    ...
    $getFlower = $conn->prepare("SELECT flowerName FROM Birthday WHERE id = ? AND owner_id = ?");
    ...
    if ($flowerRow = $flowerRes->fetch_assoc()) {
        ...
        $stmtDel = $conn->prepare("DELETE FROM Birthday WHERE id = ? AND owner_id = ?");
...
$sql = "SELECT id, flowerName, price, image_path, availability FROM Birthday WHERE owner_id = ?";
$params = [$owner_id];
$types = "i";
...
<title><?= $shopName ?> - Birthday Flowers</title>
...
<div class="main-content">
  <h2>Flower Category Birthday</h2>
...
<div class="header-top" style="margin-top:100px;">
  ...
  <a href="manageBirthday.php" class="add-flower-btn"><i class="fa-solid fa-plus"></i> Add Flower</a>
</div>
...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        <div class='product-card'>
            ...
            <a href='manageBirthday.php?edit=$id' class='edit-btn'><i class='fa-solid fa-pen'></i> Edit</a>
            <a href='#' class='delete-btn' onclick='openModal($id)'><i class='fa-solid fa-trash'></i> Delete</a>
        </div>";
    }
} else {
    echo "<p style='text-align:center; color:#ccc; grid-column:1/-1;'>No Birthday flowers found.</p>";
}
?>
</section>
