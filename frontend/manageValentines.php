...
if (isset($_GET['delete'])) {
    ...
    $getFlower = $conn->prepare("SELECT flowerName, image_path FROM Valentines WHERE id = ? AND owner_id = ?");
    ...
    if ($flowerRow = $flowerResult->fetch_assoc()) {
        ...
        $stmt = $conn->prepare("DELETE FROM Valentines WHERE id = ? AND owner_id = ?");
...
if (isset($_GET['edit'])) {
    ...
    $stmt = $conn->prepare("SELECT flowerName, price, image_path, availability FROM Valentines WHERE id = ? AND owner_id = ?");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['edit_id'])) {
        ...
        $stmt = $conn->prepare("UPDATE Valentines SET flowerName = ?, price = ?, image_path = ?, availability = ? WHERE id = ? AND owner_id = ?");
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === 0) {
            ...
            if (in_array($ext, $allowedTypes)) {
                ...
                if (move_uploaded_file($image["tmp_name"], $targetFile)) {
                    $stmt = $conn->prepare("INSERT INTO Valentines (owner_id, flowerName, price, image_path, availability) VALUES (?, ?, ?, ?, ?)");
...
$stmt = $conn->prepare("SELECT id, flowerName, price, image_path, availability FROM Valentines WHERE owner_id = ? AND flowerName LIKE ?");
...
<head>
<meta charset="UTF-8">
<title><?= $shopName ?> - Manage Valentines Flowers</title>
...
<div class="main-content">
  <div class="form-section">
    <a href="ownerCategory_Valentines.php" class="back-btn"><i class="fa-sharp fa-solid fa-arrow-left"></i> Back</a>
